...
WussComponent({
  ...
  methods: {
    ...
    _handleSelectedReset() {
      ...
      let currentOptions = options.filter(i => (i.show === true))[0] || options[0];
      if(String(currentOptions.type) === 'radio'||String(currentOptions.type) === 'custom') {
        options.map((i,idx) => {
          if(i.highlight === true) {
            i.options.map(i => {
              i.checked = false;
